...
export class TourCartComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.paramMap.subscribe((param) => {
      ...
      this.tourService.getTour(id).subscribe((data) => {
        this.tour = data;
        this.stars = new Array(this.tour.rating); //it will create an array length according to rating and it's value will be undefined
      });
    });
  }
  ...
  canDeactivate(){
    if (!this.submitStatus) {
      this.submitStatus = confirm(
        'You have not submitted a request to this tour. Any details entered will be lost. Are you sure you want to leave?'
      );
    }
  ...
  makeRequest() {
    if (
      this.tourRequest.customerName &&
      this.tourRequest.customerEmail &&
      this.tourRequest.customerPhone &&
      this.tourRequest.dateOfTravel
    ) {
