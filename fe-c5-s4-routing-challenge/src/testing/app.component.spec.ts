...
describe('AppComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [
        AppComponent, HeaderComponent, NoteComponent, SearchComponent, NoteAddComponent, NoteViewComponent, NoteEditComponent
      ],
      imports: [
  ...
  it('navigate to "/note-view" loads `Page-Not-Found` component', fakeAsync(() => {
    ...
    router.navigate(["/note-view"]).then(() => {
      ...
      expect((fixture.nativeElement as HTMLElement).innerHTML).toContain("app-page-not-found");
      flush();
    });
  }));
  ...
  it('navigate to "/notes" loads `Note-View` component', fakeAsync(() => {
    ...
    router.navigate(["/notes"]).then(() => {
      fixture.detectChanges();
      expect((fixture.nativeElement as HTMLElement).innerHTML).toContain("app-note-view");
      flush();
    });
  }));
  ...
  it('navigate to "/notes/3" loads `Note-Edit` component', fakeAsync(() => {
    ...
    router.navigate(["/notes/3"]).then(() => {
      fixture.detectChanges();
      expect((fixture.nativeElement as HTMLElement).innerHTML).toContain("app-note-edit");
      flush();
    });
  }));
});
